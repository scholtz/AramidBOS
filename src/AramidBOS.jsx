initState({
  ...
  const onChangeChainFrom = (chainId) => {
    ...
    if (
      nextChainToId != "" &&
      Object.keys(state.config.chains2tokens[chainId][nextChainToId]).length == 1
    ) {
      nextTokenFromId = Object.keys(
        state.config.chains2tokens[chainId][nextChainToId]
      )[0];
    }
    if (
      nextTokenFromId != "" &&
      Object.keys(
        state.config.chains2tokens[chainId][nextChainToId][nextTokenFromId]
      ).length == 1
    ) {
      nextTokenToId = Object.keys(
        state.config.chains2tokens[chainId][nextChainToId][nextTokenFromId]
      )[0];
    }
  ...
  const onChangeChainTo = (chainId) => {
    ...
    let nextTokenToId = "";
    if (
      Object.keys(state.config.chains2tokens[state.chainFromId][chainId])
        .length == 1
    ) {
      nextTokenFromId = Object.keys(
        state.config.chains2tokens[state.chainFromId][chainId]
      )[0];
    }
    if (
      nextTokenFromId != "" &&
      Object.keys(
        state.config.chains2tokens[state.chainFromId][chainId][nextTokenFromId]
      ).length == 1
    ) {
      nextTokenToId = Object.keys(
        state.config.chains2tokens[state.chainFromId][chainId][nextTokenFromId]
  ...
  const onChangeTokenFrom = (tokenId) => {
    let nextTokenToId = "";
    console.log(
      "state.config.chains2tokens[state.chainFromId][state.chainToId][tokenId]",
      state.config.chains2tokens[state.chainFromId][state.chainToId][tokenId]
    );
    if (
      Object.keys(
    ...
    if (state.tokenFromId !== "0x0000000000000000000000000000000000000000") {
      ...
      const msgData = {
        input: {
          ...
          root_amount: new BigNumber(appData.sourceAmount)
            .minus(appData.feeAmount)
            .toNumber()
      ...
      const tx = await appData.nearWallet?.callMethod({
        ...
        contractId: CONTRACT_ADDRESS(
          appData?.appConfiguration?.environment,
        ...
        <>
            ...
            <>
              ...
              {state.chainFromId && (
                <>
                  ...
                  >
                    ...
                    {Object.keys(
                      state.config.chains2tokens[state.chainFromId]
                    ).map((chainId) => (
                      <option value={chainId}>
                        {state.config.chains[chainId].name}
                      </option>
                  ...
                  >
                    ...
                    {Object.keys(
                      ...
                      <option value={tokenId}>
                        {
                          state.config.chains[state.chainFromId].tokens[tokenId]
                            .name
                        }
                      </option>
                    ))}
              ...
              {state.tokenFromId && (
                <>
                  ...
                  <select
                    class="form-control"
                    placeholder={state.tokenToId}
                    onChange={(e) => onChangeTokenTo(e.target.value)}
                  >
                    <option value="" selected disabled>
                    ...
                    {Object.keys(
                      ...
                      <option value={tokenId}>
                        {
                          state.config.chains[state.chainToId].tokens[tokenId]
                            .name
                        }
                      </option>
                    ))}
              ...
              {state.tokenToId && (
                <>
                  ...
                  <input
                    class="form-control"
                    placeholder={state.addressFrom}
                    onChange={(e) => onChangeAddressFrom(e.target.value)}
                    placeholder="Please select sender address"
                  />
                </>
              )}
              ...
              {state.addressFrom && (
                <>
                  ...
                  <input
                    class="form-control"
                    placeholder={state.addressTo}
                    onChange={(e) => onChangeAddressTo(e.target.value)}
                    placeholder="Please select receiver address"
                  />
                </>
              )}
              {state.addressTo && (
          ...
          {state.inReview && (
            <>
              ...
              <button
                class="btn btn-light m-2 float-end"
                onClick={() => onClickReviewBack()}
              >
                Go back
              </button>
  
              <div>
                From chain: {state.config.chains[state.chainFromId].name} (
                {state.chainFromId})
              </div>
              ...
              <div>
                From token:{" "}
                {
                  state.config.chains[state.chainFromId].tokens[state.tokenFromId]
                    .name
                }{" "}
                ({state.tokenFromId})
              </div>
              ...
              <div>
                Destination token:{" "}
                {
                  state.config.chains[state.chainToId].tokens[state.tokenToId]
                    .name
                }{" "}
                ({state.tokenToId})
              </div>
              ...
              <div>
                Destination chain: {state.config.chains[state.chainToId].name} (
                {state.chainToId})
              </div>
              <button
                class="btn btn-primary my-2"
